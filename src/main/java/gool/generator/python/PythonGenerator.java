...
CodeGeneratorNoVelocity {
	...
	public String getCode(Block block) {
		...
		if (block.getStatements().isEmpty()){
			result.append("pass");
			Log.d("<PythonGenerator - getCode(Block block)> result : " + result.toString());
		...
		Log.d("<PythonGenerator - getCode(Block block)> result : " + toReturn);
			...
			if (getName(method) == null) { // Si la méthode n'a pas encore été
				...
				if (meths.size() > 1) { // Si il y a plusieurs méthodes de même
					...
					if (someStatics && someDynamics) {
						...
						classBody += formatIndented("%-1@classmethod\n");
					}
					...
					classBody += formatIndented("%-1def %s(self, *args):%2",
							name, block);
		...
		for (Meth method : classDef.getMethods()) {
			if (!method.isMainMethod()) {
				...
				if (method.isConstructor()
					...
					classBody += formatIndented("%1",
							printMeth(method, dynamicAttributs));
				} else {
					classBody += formatIndented("%1", method);
				}
			}
		}
		if (classBody.isEmpty())
			classBody += formatIndented("%1","pass");
		body += classBody + "\n";
