...
public class AndroidCompiler extends SpecificCompiler {
	...
	public String run(File file, List<File> classPath)
		...
		try {
			...
			paramsToExecuteApk.add((new File(Settings
					.get("android_out_dir_final"))).getAbsolutePath()
					+ "//bin//AndroidProject-debug.apk");

			String commandResult = Command.exec(new File(Settings.get("android_sdk_path")),
					paramsToExecuteApk);
	...
	private void addSdkToClasspath() {
		...
		try {
			...
			String addSdkDir = "export PATH=$PATH:"
					+ Settings.get("android_sdk_location") + "/tools:"
					+ Settings.get("android_sdk_location") + "/platform-tools";
			addSdkDir.concat(Settings.get("android_sdk_location")
					+ "/platform-tools");
